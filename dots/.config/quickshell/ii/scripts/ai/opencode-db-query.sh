...
lm0=$(date -d "$(date +%Y-%m-01) -1 month" +%s 2>/dev/null || date -v-1m -v1d -v0H -v0M -v0S +%s)000
...
q_lastmonth="$sel AND time_created >= $lm0 AND time_created < $m0 GROUP BY model ORDER BY cost DESC"
...
lastmonth_json=$(sqlite3 -json "file:${db}?mode=ro" "$q_lastmonth" 2>/dev/null)
lastmonth_json=${lastmonth_json:-[]}

printf '{"today":%s,"week":%s,"month":%s,"lastMonth":%s}' "$today_json" "$week_json" "$month_json" "$lastmonth_json"
